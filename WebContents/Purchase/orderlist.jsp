 ...
 <div class="main">
  ...
  <%
   ...
   try{
         ...
         user = dao.selectById(conn, login);
          OrderDao dao2 = new OrderDao();
          order = dao2.selectById(conn, login);
          ProductDao dao3 = new ProductDao();
          pid = order.getPid();
          product = dao3.selectById(conn, pid);
        %>
   <c:set var="p" value="<%=product %>"/>
  <tr>
    <td><a href="/team_file/Product/product_detail.jsp?pid=${o.pid}"/><img src="/team_file/image/${p.category}/${p.pimage}/${p.pimage}_1.jpg" width="80" height="80"/></td>
    <td class="left">&nbsp; ${p.pname}</td>
    <td><%=order.getAmount() %></td>
    <td><strong>KRW ${p.price}원</strong></td>
    <td>0</td>
    <td><%=order.getTotalprice()/100 %></td>
    <td>기본배송</td>
    <td>KRW 3,000원</td>
