...
<div class="wrap">
  ...
  <div class="main">
  <div id="review_menu">
  <h4 align="center" style="margin-left: 300px;">남성의류</h4>

	  <h4 align="center" style="margin-left: 300px;">상품 리스트입니다.</h4>
	<%
        ...
        if(category1 !=null){
        	if(category1.equals("all")||category1.equals("null")){category1=null;}
        }
        if(sort==null){sort="";}
        try{
      		ProductDao prodDao = new ProductDao();
	        if(category1==null){	//카테고리가 없을때
            	plv = prodDao.getProductList(conn, pageNumber,sort, keyword);
            	prodCount = prodDao.selectCount(conn,keyword);
	        } else if(category1.equals("")){	 //  전체 설정 시
	        	plv = prodDao.getProductList(conn, pageNumber,sort, keyword);
            	prodCount = prodDao.selectCount(conn,keyword);
            } else{	//카테고리를 받아왔을 때
            	//plv = prodDao.getProductList(conn, pageNumber,sort);
	           	plv = prodDao.getProductListKind(conn, pageNumber, category1, sort, keyword);
            	prodCount = prodDao.selectProdCount(conn, category1,keyword);
            }
  ...
  <div class="searchDiv">
	...
	<table class="search" style="margin-left: 200px; width: 1500px">
	<tr><td width="200px">
	카테고리별 보기</td>
    	<td><select name="category" style="width: 600px; height: 40px;">
    		<option value="all">카테고리</option>
			<option value="outer">아우터</option>
			<option value="top">상 의</option>
			<option value="shirts">셔 츠</option>
			<option value="suit">수 트</option>
			<option value="bottom">하 의</option>
			<option value="trainning">트레이닝복</option>
		</select></td>
	<td width="200px">
	정렬방식</td>
    	<td><select name="sort" style="width: 200px; height: 40px;">
    		<option value="">정렬방식</option>
			<option value="order by pdate desc">신상품</option>
			<option value="order by price desc">높은가격</option>
			<option value="order by price">낮은가격</option>
			<option value="order by pname">상품명</option>
		</select></td>
		<td width="150px" align="center">상품검색</td>
		<td>	<input type="text" name ="keyword" style="width: 200px; height: 40px;"></td>
		<td>	<img src="/team_file/image/search.PNG" width="45px" height="45px" onclick="sea()"></td></tr>
		</table>
	</form>
	</div>
	<script type="text/javascript">
	function sea(){
		var target1 = frm.category.value.trim();
		var target2 = frm.sort.value.trim();
		frm.submit();
//		location.href="/team_file/Category/category.jsp?category="+target1+"&sort="+target2;
	}
	</script>
	...
	<div class="category_list">
		...
		<div class="pic">
			<div class="subpic">
			<a href="/team_file/Product/product_detail.jsp?pid=${p.pid}"><img src="/team_file/image/${p.category}/${p.pimage}/${p.pimage}_1.jpg" width="400" height="380">
			</a></div><hr>
			<div class="subpic1">
				<p>${p.pname}</p>
			</div>
	...
	<a href="#" class="bt prev"><</a>
		<c:forEach var="i" begin="1" end="${plv.pageTotalCount}">
        	<a href="/team_file/Category/category.jsp?category=<%=category1 %>&sort=<%=sort%>&page=${i}" class="num">${i}</a>
    	</c:forEach>
