...
function check_pw(){
    ...
    if(password.length < 6 || password.length>16){
        document.getElementById('pw_pro_label').innerHTML ='비밀번호의 숫자를 확인부탁드립니다.';
    ...
    if(check_SC == 0){
        document.getElementById('pw_pro_label').innerHTML = '!,@,#,$,% 특수문자를 포함해야 합니다.'
        document.getElementById('pw_pro_label').style.color='red';
        return;
    }
...
function nullCheck() {
	...
	else if (phone2.length < 1 || phone2 == null) {
		alert("전화번호를 입력해주세요");
		return false;
	}
	else if (phone3.length < 1 || phone3 == null) {
		alert("전화번호를 입력해주세요");
		return false;
	}
	else if (email.length < 1 || email == null) {
		alert("이메일을 입력해주세요");
		return false;
	}
...
<div class="main">
		<form action="/team_file/SignUp/signUp.jsp" method="post" name="frm" onsubmit="return nullCheck()">
			<div class="sign_up">
				<div class="sign_up_image">
					<img src="/team_file/image/sign_up.png" width="113" height="48">
				</div>
				<!-- <div class="sns_sign_up">
					<img src="/team_file/image/sns_sign_up.png" width="1658" height="174"
						usemap="#Map">
					<map name="Map">
						<area shape="rect" coords="20,87,330,152" href="#">
						<area shape="rect" coords="346,88,657,152" href="#">
					</map>
				</div>
 -->
				<div class="member_id">
				...
				<div class="member_pw">
					비밀번호 <input name="password" id="password" type="password" onchange="check_pw()"> (영문 대소문자/숫자/특수문자 중 2가지 이상 조합, 6자~16자) &nbsp;<span style="color:cadetblue">보안성</span> <progress id="pw_pro" value="0" max="3"></progress>&nbsp;<span id="pw_pro_label"></span>
				</div>
				<div class="member_pwCheck">
					비밀번호 확인 <input name="pwCheck" id="password2" type="password" onchange="check_pw()"> &nbsp;<span id="check"></span>
				</div>
				...
				<div class="member_tel">
					...
					</select> - <input name="phone2" type="text"> - <input name="phone3"
						type="text">
				</div>
				<div class="member_email">
					이메일 <input name="email" type="text">
					<button type="button" onclick="emailCheck()">이메일 체크</button>
				</div>
  ...
  <div class="info2">
	...
	<script type="text/javascript">
			function idCheck() {
				...
				var url = "idCheck.jsp?uid=" + id;
				window.open(url, "get", "height = 180, width = 300");
			}
			
			function passwordCheck() {
				var pw1 = document.frm.password.value;
				var pw2 = document.frm.pwCheck.value;
				if (pw1.length < 1 || pw1 == null || pw2.length < 1 || pw2==null) {
