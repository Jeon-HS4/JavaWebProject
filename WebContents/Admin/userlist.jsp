...
<div class="wrap">
  ...
  <div class="menu_header">
	...
	UserListView ulv = null;
	try{
		UserDao userDao = new UserDao();
		ulv = userDao.getUserList(conn, pageNumber);  		
  	...
  	if(ulv != null){
	%>
  <c:set var="plv" value="<%=ulv %>"/>
  <div id="detail">
	  <h3 style="margin-left: 550px;">유저관리</h3>
  <div class="btinquiry">
  <a href="/team_file/Admin/list.jsp" class="on"> 전체</a> 
  <a href="/team_file/Admin/prodlist.jsp" class="on">상품관리</a> 
  <a href="/team_file/Admin/orderlist.jsp" class="on">주문관리</a>
  <a href="/team_file/Admin/userlist.jsp" class="focus">회원관리</a>
  </div>
  <div class = "noticetablestyle">
    <table width = "1580"> 
        <tr>
            ...
            <td width = 100>이메일</td>
            <td width = 150>주소</td>
            <td width = 80>회원가입일</td>
            <td width = 150>보유 포인트</td>
            <td width="100">수정</td>
            <td width="100">삭제</td>
            </div>
        </tr>
        <%
        	int number = 1;
        %>
		<c:forEach var="p" items="${plv.userList}">
        <tr>
        	<td><%=number++ %></td>
            <td>${p.userId}</td>
            <td>${p.name}</td>
            <td>${p.phone1}-${p.phone1}-${p.phone1}</td>
            <td>${p.email }</td>
            <td>${p.address}</td>
            <td>${p.registerTime }</td>
          	<td>${p.point }</td>
        	<td><input type="button" value="수정" onclick="location.href='/team_file/Admin/userUpdateForm.jsp?uid=${p.userId }'"></td>
        	<td><input type="button" value="삭제" onclick="location.href='/team_file/Admin/userDelete.jsp?uid=${p.userId }'"></td>
        </tr>
        </c:forEach>
    </table>
</div>
<%} %>
	...
	<a href="#" class="bt prev"><</a>
		<c:forEach var="i" begin="1" end="${plv.pageTotalCount}">
        	<a href="/team_file/Admin/userlist.jsp?page=${i}" class="num">${i}</a>
    	</c:forEach>
    	<a href="#" class="bt next">></a>
    </div>
    <div class ="btwrite">
		<a href="/team_file/Admin/addUser.jsp" class="on">상품추가</a>
	</div>
