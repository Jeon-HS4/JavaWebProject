...
<div class="wrap">
  ...
  <div class="menu_header">
	...
	try{
		...
		plv = orderDao.getOrderList(conn, pageNumber);  	
  ...
  <div class = "noticetablestyle">
    <table width = "1580"> 
		...
		<c:forEach var="p" items="${plv.orderList}">
        <tr>
            <td>${p.oid}</td>
            <td>${p.mid }</td>
            <td>${p.pid }</td>
            ...
            <td>${p.address }</td>
          	<td>${p.totalprice }</td>
        	<td><input type="button" value="수정" onclick="location.href='/team_file/Admin/orderUpdateForm.jsp?oid=${p.oid }'"></td>
        	<td><input type="button" value="삭제" onclick="location.href='/team_file/Admin/orderDelete.jsp?oid=${p.oid }'"></td>
        </tr>
        </c:forEach>
    </table>
</div>
